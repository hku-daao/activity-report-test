...
export function JournalsListPage({ user }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void listJournalsForUser(user.uid).then((r) => {
      if (cancelled) return
      if (!r.ok) {
        setError(r.message)
        setRows([])
    ...
    <div className="dashboard-page">
      ...
      <main className="dashboard-main">
        <section className="dashboard-panel activity-dashboard">
          <h2 className="dashboard-section-title">Your daily journals</h2>
          <p className="activity-muted journal-list-intro">
            Each journal is tied to you as creator and to one calendar day. Open
            a row to continue editing.
          </p>
          ...
          {loading ? (
            ...
            <p className="activity-muted">No journals yet. Use Daily Journal on the home page to start.</p>
          ) : (
                ...
                <li key={row.id}>
                  ...
                  >
                    ...
                    <span className="activity-report-meta">
                      Day {row.journal_date} · Updated {formatWhen(row.updated_at)}
                      {' · '}
